...
# This script plays a rosbog and listens for depth frame from ESS depth
# pipeline then validate the depth of the white board in the rosbag frames.
...
def get_args():
    parser = argparse.ArgumentParser(description='ESS Depath Validation')
    parser.add_argument('--topic', default='depth', help='Depth topic.')
    parser.add_argument('--depth_nomial',
                        default=DEPTH_METER,
...
class DepthValidation(Node):
    ...
    def __init__(self, args):
        ...
        self._disp_sub = self.create_subscription(Image, args.topic,
                                                  self.depth_callback, 10)
        self.play_rosbag()

    def play_rosbag(self):
        left_image_topic = '/' + self.args.camera + '/left/image_compressed '
        left_info_topic = '/' + self.args.camera + '/left/camerainfo '
        right_image_topic = '/' + self.args.camera + '/right/image_compressed '
        right_info_topic = '/' + self.args.camera + '/right/camerainfo '
...
def main():
    ...
    try:
        ...
        rclpy.logging.get_logger('depth_validation').info('Done')
