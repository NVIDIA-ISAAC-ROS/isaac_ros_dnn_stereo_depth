...
gxf_result_t VideoBufferThresholder::tick() {
  ...
  gxf::Entity image_entity = GXF_UNWRAP_OR_RETURN(image_input_.get()->receive());
  gxf::Entity mask_entity = GXF_UNWRAP_OR_RETURN(mask_input_.get()->receive());
  gxf::Entity output_entity;

  gxf::Handle<gxf::VideoBuffer> mask_video_buffer =
      GXF_UNWRAP_OR_RETURN(mask_entity.get<gxf::VideoBuffer>());
  ...
  if (!IsCameraMessage(image_entity).value()) {
    ...
    image_video_buffer = GXF_UNWRAP_OR_RETURN(image_entity.get<gxf::VideoBuffer>(name));
    const gxf::VideoBufferInfo& video_info = image_video_buffer->video_frame_info();
    output_entity = GXF_UNWRAP_OR_RETURN(gxf::Entity::New(context()));
    tx_video_buffer = GXF_UNWRAP_OR_RETURN(output_entity.add<gxf::VideoBuffer>(name));
    if (video_info.color_format == gxf::VideoFormat::GXF_VIDEO_FORMAT_D32F) {
      GXF_RETURN_IF_ERROR(AllocateUnpaddedVideoBuffer<gxf::VideoFormat::GXF_VIDEO_FORMAT_D32F>(
        ...
        GXF_RETURN_IF_ERROR(AllocateUnpaddedVideoBuffer<gxf::VideoFormat::GXF_VIDEO_FORMAT_RGB>(
        tx_video_buffer, video_info.width, video_info.height,
        gxf::MemoryStorageType::kDevice, allocator_));
    }
    AddInputTimestampToOutput(output_entity, image_entity);
  } else {
    CameraMessageParts image_message = GXF_UNWRAP_OR_RETURN(GetCameraMessage(image_entity));
  ...
  GXF_RETURN_IF_ERROR(validateImageMessage(image_video_buffer));
  GXF_RETURN_IF_ERROR(validateMaskMessage(image_video_buffer, mask_video_buffer));
  GXF_RETURN_IF_ERROR(thresholdImage(image_video_buffer, mask_video_buffer, tx_video_buffer));
  GXF_RETURN_IF_ERROR(gxf::ToResultCode(masked_output_->publish(output_entity)));
...
gxf_result_t VideoBufferThresholder::thresholdImage(
  ...
  gxf::Handle<gxf::VideoBuffer> output_video_buffer) const {
  const gxf::VideoBufferInfo& video_info = image_video_buffer->video_frame_info();
