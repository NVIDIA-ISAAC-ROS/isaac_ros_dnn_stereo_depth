...
template <typename K, int N>
gxf::Expected<::nvidia::isaac::CpuTensorConstView<K, N>>
ToIsaacCpuTensorView(const gxf::Tensor& tensor) {
  if ((tensor.storage_type() != gxf::MemoryStorageType::kHost) &&
      (tensor.storage_type() != gxf::MemoryStorageType::kSystem)) {
    GXF_LOG_ERROR("Tensor does not have host or system storage type");
    return gxf::Unexpected{GXF_INVALID_DATA_FORMAT};
  ...
  return ::nvidia::isaac::CreateCpuTensorConstViewFromData, N> (
      maybe_ptr.value(), dimensions.prod(), dimensions);
}
...
template <typename K, int N>
gxf::Expected<::nvidia::isaac::GpuTensorConstView<K, N>>
ToIsaacGpuTensorView(const gxf::Tensor& tensor) {
  if (tensor.storage_type() != gxf::MemoryStorageType::kDevice) {
    GXF_LOG_ERROR("Tensor does not have device storage type");
    return gxf::Unexpected{GXF_INVALID_DATA_FORMAT};
  }
  ...
  return ::nvidia::isaac::CreateGpuTensorConstViewFromData<K, N>(maybe_ptr.value(),
                                                                 dimensions.prod(),
                                                                 dimensions);
}
